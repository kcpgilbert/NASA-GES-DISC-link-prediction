...
def add_platform(tx, platform_data: dict, platform_id) -> str:
    ...
    platform_instrument_longname = platform_data.get("Instruments", [{}])[0].get("LongName")  # Use get to handle missing keys
    current_batch = [
        platform_globalId,
        platform_shortName,
        platform_longName,
    ...
    tx.run(
        ...
        ON CREATE SET p.shortName = $shortName, p.longName = $longName, p.instrument_longname = $instrument_longname
        """,
        globalId=platform_globalId,
        shortName=platform_shortName,
        longName=platform_longName,
        instrument_longname=platform_instrument_longname
    )
    return platform_id
...
with driver.session() as session:
    ...
    print(f"Total platforms indexed: {count}")