...
def add_platform(batch: list, platform_data: dict, platform_id) -> str:
    ...
    try:
        platform_instrument_longname = platform_data["Instruments"][0]["LongName"]
    except KeyError as error:
        platform_instrument_longname = None
        print(f"Platform {platform_instrument_longname} does nt have {error}!")

    current_batch = [
        platform_globalId,
        platform_shortName,
        platform_longName,
        platform_instrument_longname
...
keys = ["globalId", "shortName", "longName", "instrument_longname"]
batch = []
...
merge_nodes(graph.auto(), batch, merge_key=("Platform", "globalId"), keys=keys)
print(f"Total plforms indexed: {graph.nodes.match('Platform').count()}")
